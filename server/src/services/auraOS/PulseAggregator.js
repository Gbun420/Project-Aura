...
export class PulseAggregator {
     ...
     */
    static async getEmployerPulse(db, employerId) {
        ...
        const results = await Promise.all(activeIntroductions.map(async (intro) => {
    ...
    static calculateRemainingDays(expiry) {
        const diff = new Date(expiry).getTime() - new Date().getTime();
        return Math.max(0, Math.ceil(diff / (1000 * 60 * 60 * 24)));
    }
}
//# sourceMappingURL=PulseAggregator.js.map